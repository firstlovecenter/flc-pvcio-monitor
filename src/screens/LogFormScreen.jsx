...
export default function LogFormScreen() {
  ...
  const governorshipName = user?.governorship?.name || user?.activeChurch?.name || ''

  return (
    <div
      className='min-h-dvh'
      style={{
        background: 'radial-gradient(130% 90% at 50% -10%, #1A2450 0%, #101528 50%, #0C0F1A 100%)',
        color: 'var(--text)',
      }}
    >
      {/* Sticky header */}
      ...
      >
        <button
          ...
          style={{ width: 36, height: 36, background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--text)' }}
        >
          ←
        </button>
        <div>
      ...
      <div className='px-4 pb-10'>
        <div className='mx-auto w-full max-w-md flex flex-col gap-5'>
          ...
          >
            <h1 className='m-0 text-xl font-semibold leading-snug'>{activity.name}</h1>
            {(unitName || governorshipName) && (
              <p className='m-0 mt-1 text-sm font-mono' style={{ color: 'var(--muted)' }}>
            ...
            {activity.desc && (
              <span
                ...
                style={{ background: 'rgba(79,127,255,0.15)', color: 'var(--accent)' }}
              >
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
            ...
            <button
              ...
              style={{ background: submitting ? 'var(--border)' : 'var(--accent)', color: '#fff', border: 'none', opacity: submitting ? 0.7 : 1 }}
            >
