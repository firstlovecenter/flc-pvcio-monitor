    ...
    getWeeklySummary(user.userId, prevIsoWeek)
      .then((existing) => {
        ...
        const weekLogs = recentLogs.filter(
          (log) => log.iso_week === prevIsoWeek && log.type === 'activity',
        )
        const logsCount = weekLogs.length
        const loggedIds = new Set(weekLogs.map((l) => l.activity_id))
        const recurring = getActivitiesForLevel(user.level).filter(
          (a) => a.freq === 'weekly',
        )
      ...
      .catch((err) =>
        console.error('[HomeScreen] weekly summary check:', err.message),
      )
  ...
  function handleSummaryDismiss() {
    if (weeklySummaryData?.isoWeek) {
      sessionStorage.setItem(
        `weeklySummaryDismissed:${weeklySummaryData.isoWeek}`,
        '1',
      )
    }
              ...
              >
                {(user.churchContexts || []).map((ctx) => (
                  <option key={`${ctx.level}:${ctx.id}`} value={ctx.id}>
                    {ctx.name} (
                    {ctx.level === 'overseer' ? 'Council' : ctx.level})
                  </option>
                ))}
              </select>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className='flex items-center justify-between gap-2'>
                        ...
                        >
                          {log.submitted_at &&
                          !isNaN(new Date(log.submitted_at))
