...
export default function LeaderDetailScreen() {
  ...
  const activeData = data ? (week === 'last' ? data.lastWeek : data.thisWeek) : null
        ...
        {leader && (
          <div className='mb-4'>
            <p className='text-xs' style={{ color: 'var(--muted)' }}>
              {leader.bacentaName ?? leader.governorshipName ?? leader.councilName}
              {' · '}
          ...
          <div className='rounded-2xl overflow-hidden animate-pulse'
            ...
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className='px-4 py-4'
                ...
                <div className='h-3 rounded w-3/4 mb-1' style={{ background: 'var(--border)' }} />
          ...
          return (
            <>
              ...
              {notExpected.length > 0 && (
                <>
                  <p className='text-xs font-semibold uppercase tracking-widest mb-2 mt-4'
